...
class GreedyBustersAgent(BustersAgent):
    ...
    def chooseAction(self, gameState):
        ...
        livingGhostPositionDistributions = [beliefs for i,beliefs
                                            ...
                                            if livingGhosts[i+1]]
        "*** YOUR CODE HERE ***"
        closestGhost = livingGhostPositionDistributions[0]
        minDistance = self.distancer.getDistance(livingGhostPositionDistributions[0].argMax(), pacmanPosition)
        for i in range(1, len(livingGhostPositionDistributions)):
            ghost = livingGhostPositionDistributions[i]
            if self.distancer.getDistance(ghost.argMax(), pacmanPosition) < minDistance:
                minDistance = self.distancer.getDistance(ghost.argMax(), pacmanPosition)
                closestGhost = ghost
        position = closestGhost.argMax()

        nextPositions = util.Counter();
        legalActions = gameState.getLegalPacmanActions()
        for action in legalActions:
            nextPositions[action] = -self.distancer.getDistance(Actions.getSuccessor(pacmanPosition, action), position)
        return nextPositions.argMax()
