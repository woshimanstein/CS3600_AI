...
class ExactInference(InferenceModule):
    ...
    def applyTransitionModel(allPossible, emissionModel):
        new_belief = util.Counter();
        for p1 in self.legalPositions:
            for p2 in self.legalPositions:
                new_belief[p2] = allPossible[p1] * emissionModel[util.manhattanDistance(self.pacmanPosition, p1)]
        return new_belief.normalize();
...
class ParticleFilter(InferenceModule):
    ...
    def initializeUniformly(self, gameState):
        ...
        numPerPosition = self.numParticles / numLegalPositions
        for position in self.legalPositions:
            for i in range(0, numPerPosition):
                self.particle.append(position)
    ...
    def observe(self, observation, gameState):
        ...
        if noisyDistance == None:
            ...
            for i in range(0, self.numParticles):
                self.particle.append(self.getJailPosition())
        else:
            weight = self.getBeliefDistribution()
            for instance in weight:
                weight[instance] = weight[instance] * emissionModel[util.manhattanDistance(pacmanPosition, instance)]
            weight.normalize()
    ...
    def elapseTime(self, gameState):
        ...
        "*** YOUR CODE HERE ***"
        nextStep = list()
        particleTimes = util.Counter()
        for position in self.particle:
            particleTimes[position] += 1
        for position in particleTimes.sortedKeys():
            transition = util.Counter()
            newPosDist = self.getPositionDistribution(self.setGhostPosition(gameState, position))
            for nextPosition in self.legalPositions:
                transition[nextPosition] = newPosDist[nextPosition]
            transition.normalize()
            if transition.totalCount() == 0:
                self.initializeUniformly(gameState)
                return
            for i in range(0, particleTimes[position]):
                nextStep.append(util.sample(transition))
        self.particle = nextStep
...
class JointParticleFilter:
    ...
    def initializeParticles(self):
        ...
        "*** YOUR CODE HERE ***"
        self.particles = list()
        itr = itertools.permutations(self.legalPositions,  self.numGhosts)
        productList = list()
        for product in itr:
            productList.append(product)
        random.shuffle(productList)
        for i in range(0, self.numParticles):
            self.particles.append(productList[i % len(productList)])
        ...
        for particle in self.particles:
            ...
            for i in range(self.numGhosts):
                if noisyDistances[i] == None:
                    ...
                    joint *= emissionModels[i][distance]
            allPossible[particle] += joint
